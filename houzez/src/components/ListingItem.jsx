...
export default function ListingItem({ listing, id, onEdit, onDelete }) {
  return (
    <li className="relative bg-white flex flex-col justify-between items-center shadow-md hover:shadow-xl rounded-xl overflow-hidden transition-shadow duration-150 m-[10px]">
      ...
      {/* delete icon  */}
      {onDelete && <FaTrash className="absolute bottom-2 right-2 h-[14px] cursor-pointer text-red-500 text-sm" onClick={() => onDelete(listing.id)} />}

      {/* edit icon  */}
      {onEdit && <MdEdit className="absolute bottom-2 right-7 h-4 cursor-pointer text-sm hover:text-blue-500" onClick={() => onEdit(listing.id)} />}
    </li>
  );
}
